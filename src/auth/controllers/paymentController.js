...
exports.simulateWalletPayment = asyncErrorHandler(async (req, res, next) => {
  const { wallet_address, amount, timestamp } = req.body;
  ...
  res.status(200).json({
    success: true,
    message: "Simulated wallet payment received",
    data: { wallet_address, amount, timestamp },
  });
});
